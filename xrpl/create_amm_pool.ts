...
import type { AMMCreate, TrustSet, AccountSet, AMMDeposit } from 'xrpl'
...
async function addLiquidity(client: Client, wallet: any) {
    ...
    const response = await client.submit(depositTx, { wallet })
    console.log(`✅ Added liquidity: https://testnet.xrpl.org/transactions/${response.result.tx_json.hash}`)
}
    ...
    try {
        ...
        try {
            await client.request({
                command: "amm_info",
                asset: { currency: "SGD", issuer: SGD_ISSUER },
                asset2: { currency: RLUSD_HEX, issuer: RLUSD_ISSUER }
            })
            poolExists = true
        } catch (e) {
            poolExists = false
        ...
        if (poolExists) {
            ...
            await addLiquidity(client, wallet)
        } else {
            // Create the pool
            console.log("\nCreating AMM pool...")
